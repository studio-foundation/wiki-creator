...
def _predictions_from_file(path: Path) -> dict[tuple[str, str], str | None]:
    data = json.loads(path.read_text(encoding="utf-8"))
    relationships = data.get("relationships", data) if isinstance(data, dict) else data
    return predictions_from_relationships(relationships)


def _predictions_from_live_run(gold, book_yaml: Path | None) -> dict[tuple[str, str], str | None]:
    ...
    for gp in gold:
        ...
        if clf and not clf.get("error"):
            ...
            continue
        print(f" → {predictions[gp.key] or 'null'}", file=sys.stderr)
    return predictions


def _check_thresholds(metrics: dict, args) -> list[str]:
...
def main() -> None:
    ...
    if args.predictions:
        predictions = _predictions_from_file(args.predictions)
    else:
        predictions = _predictions_from_live_run(gold, args.book)

    metrics = score(gold, predictions)
    report = render_report(args.fixture.stem, metrics)
