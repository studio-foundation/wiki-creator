...
from scripts.generate_series_arc import run_for_series
...
def build_assembly(series_dir: Path | str, *, timeout: int = 120, skip_arc: bool = False) -> dict:
    ...
    arc = None if skip_arc else run_for_series(series_dir, timeout=timeout)
    return {
...
def main() -> None:
    ...
    if args.series:
        ...
        # from disk, never from the payload (STU-455).
        series_dir = studio_io.paths_from_payload(studio_io.read_payload()).series_dir

    assembly = build_assembly(series_dir, timeout=args.timeout, skip_arc=args.no_arc)
