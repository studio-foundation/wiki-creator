...
Verdict and numbers: [results.md](results.md) (generated by `report.py`; the
prose in it is [recommendation.md](recommendation.md)).

**The short version: fix the window, buy nothing.** Sliding it over the chapter
instead of over an entity-ordered list takes detection F1 from 0.200 to 0.507 and
lands level with GLiREL, for a change to how one list is built. The two bugs this
spike found (STU-536, STU-537) block the ticket's own question.
...
## Run
    ...
    # 3. The oracle roster — see "Design" for why the shipped one cannot be used
    python retype_roster.py \
      --processing-output ../../library/christopher_paolini/inheritance/processing_output/01_eragon

    # 4. The implicit/explicit split (no model, ~4s)
    python explicit_pairs.py --roster roster_oracle.json --out explicit_pairs_oracle.json

    # 5. Gold — 60 chapters through the claude-code CLI, ~15 min
    python build_gold.py --roster roster_oracle.json --explicit-pairs explicit_pairs_oracle.json

    # 6. Arms
    python runners/run_cooccurrence.py --arm shipped \
      --processing-output ../../library/christopher_paolini/inheritance/processing_output/01_eragon
    python runners/run_cooccurrence.py --arm fixed --roster roster_oracle.json
    python runners/run_glirel.py --roster roster_oracle.json --device cuda --threshold 0.15
    python runners/run_llm_schema.py --roster roster_oracle.json --model claude-sonnet-5

    # 7. Report (recommendation.md is inlined; results.md is generated)
    python report.py
