# Relation-discovery bake-off (STU-467, then STU-540)
...
1. **STU-467, the spike — fix the window, buy nothing.** Sliding it over the
   ...
   0.507 and lands level with GLiREL, for a change to how one list is built. The
   two bugs it found (STU-536, STU-537) blocked the ticket's own question.
   [results.md](results.md) (generated by `report.py`; its prose is
   [recommendation.md](recommendation.md)).

2. **STU-540, after those landed — buy schema-guided discovery, and stop losing
   half the graph in the classifier.** Decided against a *human* gold rather than
...
| arm | what it tests |
|---|---|
| `cooccurrence_shipped` | production before STU-536 — the artifact a pre-fix run left behind |
| `cooccurrence_fixed` | discovery as it runs today (STU-536 landed it in the stage) |
| `cooccurrence_classified` | **production, end to end** — discovery + the per-pair classifier + STU-501's filter |
...
`cooccurrence_classified` is a STU-540 addition, and it is the one that decides.
The other co-occurrence arms stop at discovery, and discovery is not what ships: a
pair the classifier leaves untyped is omitted from every reader-facing surface
(STU-501). Scoring raw co-occurrence against a schema-guided LLM compares half a
pipeline to a whole one — it charges the window for pairs the classifier would have
dropped anyway. It turns out the classifier drops far more than that; see
...
## Run
    ...
    python runners/run_llm_schema.py --roster roster.json --explicit-pairs explicit_pairs.json \
      --model claude-sonnet-5
    python runners/run_glirel.py --roster roster.json --device cuda --threshold 0.15
    ...
    # 5. The human gold (STU-540) — 111 keypresses, ~30 min, resumable
    python adjudicate.py --corpus corpus.jsonl --roster roster.json
    python vote.py
    python score_adjudication.py --votes votes.json

    # 6. The spike's report against the LLM gold (recommendation.md is inlined)
    python report.py

**`retype_roster.py` and `build_gold.py` are not in that list, and that is the
point.**

The oracle roster existed to undo spaCy's typing of invented names — it is what
put `Garrow` back among the people and `Varden` out of them. STU-537 does that
upstream now, with GLiNER, so `build_corpus.py` reports `mistyped: 0` on the
*shipped* roster. Every arm runs on what production actually produces, which
removes an LLM from the loop and retires the README's "the roster is dirty and
stays dirty" design note along with it. `roster_oracle.json` is still buildable;
nothing needs it.

The LLM gold is not built at all. It was caveat 1 — an LLM grading an LLM — and
the STU-540 adjudication replaced it with a human, by asking only where the arms
disagree. The `--arm shipped` line is gone for a different reason: STU-536 landed
the fix in the stage, so re-running the pipeline overwrites the artifact that arm
reads. It is history, not a baseline.

Everything runs through `claude -p`, which bills the subscription the pipeline
already bills — the Studio config is `provider: claude-code`. No API key needed.
`run_glirel.py` wants the GPU: it is ~12.5s per sentence on CPU, which will not
cross 60 chapters.

GLiREL's `--threshold 0.15` is not its default. At the shipped default of 0.5 it
emits **nothing at all** — 0 relations across 42 chapters, because its scores top
out at 0.205 on this label set. The threshold here was picked by looking at the
gold, which makes that arm's number optimistic and is caveat 2 in the report.
